...
@customElement('exm-sortable-demo')
  ...
  render() {
    return html`
      <div class="main centered">
        <div>
          <h2>List (Animated)</h2>
          <exm-sortable animation-enabled orientation="vertical" @dom-order-change="${this.orderChange}">
            <ul>
              ${this.users.map((user) => {
                return html`
                  <li>
          ...
          <exm-sortable item-selector="div.box" animation-enabled @dom-order-change="${this.orderChange}">
            <div class="boxes">
              ${this.users.map((user) => {
